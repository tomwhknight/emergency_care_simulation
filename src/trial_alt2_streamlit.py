# src/trial_alt2_streamlit.py

import numpy as np
import pandas as pd

from src.model_alt2 import AltModel2


class AltTrial2Streamlit:
    """
    Direct-to-consultant trial runner for Streamlit/hosted use.
    ...
    def run(self, run_number: int, dt_threshold: float | None = None, progress_bar=None):
        # Compatibility only; AltModel2 does not use this unless your wider code does.
        ...
        for i in range(run_number):
            run_idx = i + 1
            print(f"[ALT2] Run {run_idx}/{run_number} | scenario={scenario_name}")
